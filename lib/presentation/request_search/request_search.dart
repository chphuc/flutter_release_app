...
class _RequestSearchScreenState extends State<RequestSearchScreen> {
  final RequestSearchStore _requestSearchStore = getIt<RequestSearchStore>();
  ...
  static const int maxResultCountDefault = 5;
  static const int loadMoreCount = 2;
  ...
  void _fetchRequestStatus() {
    _requestSearchStore.getRequestStatuses();
  }

  void _fetchProcessingStep() {
    _requestSearchStore.getProcessingSteps();
  }
  ...
  void _fetchRequestType() {
    _requestSearchStore.getRequestTypes();
  }
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: Observer(
              builder: (context) {
                if (_requestSearchStore.requestLoading && isFirstTimeLoading) {
                  return CustomProgressIndicatorWidget();
                } else {
                  if (_requestSearchStore.requestList != null &&
                      !_requestSearchStore.requestList!.isEmpty) {
    ...
    return DropListDownWidget(
      ...
      items: [
        ...
        ...generateDropdownItems(
            _requestSearchStore.requestStatusList?.data ?? []),
      ],
  ...
  Widget _buildDepartmentField() {
    return DropListDownWidget(
      ...
      items: [
        DropdownItem('Tất cả', ''),
        ...generateDropdownItems(
            _requestSearchStore.requestDepartmentList?.data ?? [])
      ],
  ...
  Widget _buildCurrentStepField() {
    return DropListDownWidget(
      ...
      items: [
        DropdownItem('Tất cả', ''),
        ...generateDropdownItems(
            _requestSearchStore.processingStepList?.data ?? [])
      ],
  ...
  Widget _buildTypeRequestField() {
    return DropListDownWidget(
      ...
      items: [
        DropdownItem('Tất cả', ''),
        ...generateDropdownItems(
            _requestSearchStore.requestTypeList?.data ?? [])
      ],
  ...
  Widget _requests() {
    return ListView.builder(
      ...
      itemCount: _requestSearchStore.requestLoading
          ? _requestSearchStore.requestList!.requests.length + 1
          : _requestSearchStore.requestList!.requests.length,
      itemBuilder: (context, index) {
        if (index < _requestSearchStore.requestList!.requests.length) {
          final request = _requestSearchStore.requestList!.requests[index];
          return Padding(
  ...
  Widget _handleErrorMessage() {
    return Observer(
      builder: (context) {
        if (_requestSearchStore.errorStore.errorMessage.isNotEmpty) {
          return _showErrorMessage(_requestSearchStore.errorStore.errorMessage);
        }
